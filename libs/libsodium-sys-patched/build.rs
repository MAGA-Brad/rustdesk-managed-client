...
// Managed-client patch: Cargo gives every build script the SAME environment
// regardless of whether it's compiling this crate for HOST (e.g. a build.rs
// dependency, always native-arch) or for TARGET (the real cross-compile,
// e.g. aarch64-pc-windows-msvc). A plain SODIUM_LIB_DIR can't tell those two
// invocations apart, which matters here specifically because ARM64 Windows
// has no libsodium prebuilt/from-source path through this crate - only the
// vcpkg-built static lib works - while the native x64 host side must NOT be
// pointed at that ARM64 lib (architecture mismatch at link time). A
// `SODIUM_LIB_DIR_<TARGET_WITH_UNDERSCORES>` variable (checked first, plain
// SODIUM_LIB_DIR as fallback) lets the two be set independently.
fn env_target_scoped(name: &str) -> Option<String> {
    if let Ok(target) = env::var("TARGET") {
        let scoped = format!("{}_{}", name, target.replace(['-', '.'], "_"));
...
fn main() {
    ...
    let lib_dir_isset = env_target_scoped("SODIUM_LIB_DIR").is_some();
...
fn find_libsodium_env() {
    let lib_dir = env_target_scoped("SODIUM_LIB_DIR").unwrap(); // cannot fail, main() already checked
